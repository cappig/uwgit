...
use super::util::{PAGE_SIZE, RepoRequestContext, build_pager, display_time, render_template};
...
pub async fn log(
    ...
    render_template(move || {
        ...
        let commit_hashes = commits_raw
            ...
            .map(|c| c.hash.clone())
            .collect::<Vec<_>>();
        let refs_by_commit = git::refs_for_commits(&ctx.repo, &commit_hashes).unwrap_or_default();
